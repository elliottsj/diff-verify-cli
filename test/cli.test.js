...
const pathExists = fs.promises.access;
...
"
  ...
  Usage
    $ diff-verify -p <path|glob> [--tmp-dir <path>] [--dry-run] -- <command> [...args]

  Options
    --tmp-dir   Create '.tmp' files in the specified directory.
...
test('exit 0 when generated files in folder unchanged using --tmp-dir', async () => {
  await mkdir('test/generated', { recursive: true });
  await writeFile('test/generated/hello.txt', 'hello');
  await writeFile('test/generated/world.txt', 'world');
  ...
  expect(result.exitCode).toBe(0);
  await expect(pathExists(tmpDir)).rejects.toThrow();
});

test('exit 1 when generated files in folder changed using --tmp-dir', async () => {
  await mkdir('test/generated', { recursive: true });
  await writeFile('test/generated/hello.txt', 'foo');
  await writeFile('test/generated/world.txt', 'bar');
  const tmpDir = 'test/tmp';
