...
const getTmpFileName = (file, tmpDir) => {
  const basename = path.basename(file);
  return tmpDir ? path.join(tmpDir, `${basename}.tmp`) : `${file}.tmp`;
};
...
(async () => {
  ...
  for (const file of files) {
    const tempFile = getTmpFileName(file, tmpDir);
    if (isPathCwd(file)) {
